# Dynamic per-app tools — Preview app and `serve` (M4)
...
3. **Preview app**: sandboxed `<iframe>` rendering `~/site/` via an in-page virtual server (Service Worker if simple, else blob URLs + `<base>` rewriting for relative assets). Address bar shows `verbos://site/`. Live-reloads on FS change under the served root. Captures the iframe's console (postMessage shim injected into served HTML) into a ring buffer.
