# Desktop shell — scaffold, window manager, first live tools (M1)

Goal: a running VerbOS desktop at `bun run dev` (in `web/`) where an agent can already open, move, and close windows via WebMCP — and the human sees it happen.
...
7. **Tools** (`src/tools/`): registry module wrapping `@nekuda/webmcp-sdk` (`defineTool`/`registerTools`). At boot register: `app_open`, `app_close`, `app_list`, `window_focus`, `window_move`, `window_resize`, `sys_status`, `screensaver_wake`. Every tool call: wakes screensaver if active, appends to OS event log with `source: 'agent'`, and returns useful data (e.g. `app_open` returns `{pid, appId, rect}`; `app_list` returns the process table; `sys_status` returns `{hostname: 'guest@verbos', uptime_s, processes, agent_seen: bool}`).
