...
WebMCP Computer is a full computer that lives inside one browser tab, built for agents as first-class citizens. Files, processes, windows, applications, and the terminal are exposed directly as WebMCP tools. The agent operates the machine natively. The human watches the same desktop, live, and can step in at any moment.
...
- **Shell** is just-bash, a bash interpreter in JavaScript with coreutils, awk, and jq. Network commands and language runtimes stay off in the browser shell and live in the opt-in cloud container.
...
Stack: Vite, React 18, TypeScript strict, hand-rolled CSS. No UI kit. 339 unit tests and a native-Chrome WebMCP end-to-end suite.
