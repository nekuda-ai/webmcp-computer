...
const PERSISTENCE_WARNING = "WebMCP Computer session persistence unavailable; cross-restart persistence disabled";
...
export function restoreSessionFromStorage(storage?: Storage): boolean {
  if (restoreLatch.invoked) {
    if (import.meta.env.DEV) console.warn("WebMCP Computer session restore already ran; ignoring duplicate invocation");
    return false;
  }
  ...
  if (serialized === null && storage === undefined && typeof window !== "undefined") {
    try {
      serialized = window.sessionStorage.getItem(SESSION_STORAGE_KEY);
      if (serialized !== null && target) target.setItem(SESSION_STORAGE_KEY, serialized);
    } catch {
      // Legacy per-tab storage is only a one-time migration source.
    }
  }
